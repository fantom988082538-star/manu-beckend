...
// ВАЖНО: этому роуту нужен "сырой" (необработанный) текст тела запроса,
// чтобы правильно проверить подпись — см. подключение express.raw() в server.js
router.post('/alu', (req, res) => {
  const signature = req.headers['x-webhook-signature'];
  const timestamp = req.headers['x-webhook-timestamp'];
  const rawBody = req.body.toString('utf-8'); // req.body здесь Buffer благодаря express.raw()
