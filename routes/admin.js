...
// Изменить основные данные игры (название, нужен ли сервер, код у поставщика)
router.patch('/games/:key', requireRole('super_admin'), async (req, res) => {
  const { title, needsServer, supplierGameCode, idLabel, idPlaceholder, serverLabel, serverPlaceholder } = req.body;
  const db = await readDB();
  const game = db.games.find(g => g.key === req.params.key);
  if (!game) return res.status(404).json({ error: 'Игра не найдена' });
...
router.post('/games', requireRole('super_admin'), async (req, res) => {
  const { key, title, needsServer, supplierGameCode, idLabel, idPlaceholder, serverLabel, serverPlaceholder, packs } = req.body;
  const db = await readDB();
